...
export default function TenantLayout({
  ...
  const impersonatedData = isImpersonating_
    ? JSON.parse(sessionStorage.getItem("urbanrent_impersonate_data") || "{}")
    : null;
  ...
  const navItems = [
    ...
    {
      label: "Dashboard",
      path: resolvePath("/tenant/dashboard"),
      icon: <LayoutDashboard size={20} />,
    },
    {
      label: "Properties",
      path: resolvePath("/tenant/home"),
      icon: <Home size={20} />,
    },
    {
      label: "Favourites",
      path: resolvePath("/tenant/favourites"),
      icon: <Heart size={20} />,
    },
    {
      label: "Applications",
      path: resolvePath("/tenant/applications"),
    ...
    {
      label: "All Listings",
      path: resolvePath("/tenant/properties"),
      icon: <List size={20} />,
    },
    {
      label: "Search",
      path: resolvePath("/tenant/search"),
      icon: <Search size={20} />,
    },
    {
      label: "Payments",
      path: resolvePath("/tenant/payments"),
      icon: <CreditCard size={20} />,
    },
    {
      label: "News & Blogs",
      path: resolvePath("/tenant/blogs"),
  ...
  const handleSearchNavigate = () => {
    if (!location.pathname.includes("/search")) {
      navigate(
        resolvePath(`/tenant/search?q=${encodeURIComponent(currentQuery)}`),
      );
    }
  };
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-dark-50 font-sans">
        ...
        bottomContent={(expanded) => (
          <>
            {isImpersonating_ ? (
              ...
              >
                <div
                  className={`rounded-full border border-indigo-500 overflow-hidden flex-shrink-0 shadow-[0_0_15px_rgba(99,102,241,0.5)] bg-dark-50 ${expanded ? "w-10 h-10" : "w-10 h-10"}`}
                >
                  {impersonatedData?.avatar ? (
                    <img
                      src={impersonatedData.avatar}
                      alt="User"
                  ...
                  >
                    <UserButton
                      ...
                      appearance={{
                        elements: {
                          avatarBox: expanded
                            ? "w-10 h-10 border border-dark-200 shadow-sm"
                            : "w-10 h-10 border border-dark-200 shadow-sm",
                        },
                      }}
                    />
      ...
      <div
        className={`flex-1 overflow-y-auto custom-scrollbar flex flex-col min-w-0 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] pr-4 ${sidebarOpen ? "ml-[288px]" : "ml-[112px]"} ${isImpersonating_ ? "mt-8" : ""}`}
      >
            ...
            <header className="bg-white shadow-sm border border-dark-100 rounded-xl px-4 sm:px-6 py-2.5 flex flex-col sm:flex-row items-center justify-between gap-4 w-full relative">
              ...
              {breadcrumbs ? (
                ...
                <div className="flex-shrink-0 font-bold text-dark-900 flex items-center gap-2 text-lg">
                  Welcome back,{" "}
                  {isImpersonating_ && impersonatedData?.name ? (
                    <span className="text-indigo-600">
                      {impersonatedData.name.split(" ")[0]}
                    </span>
                  ) : user?.firstName ? (
