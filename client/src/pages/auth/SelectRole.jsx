...
const roles = [
  {
    ...
    description:
      "Browse verified properties, schedule visits, sign digital agreements, and manage your rental journey.",
    icon: (
      <svg
        className="w-8 h-8"
        fill="none"
  ...
  },
  {
    ...
    description:
      "List your properties, find quality tenants, manage bookings, and handle rent collection seamlessly.",
    icon: (
      <svg
        className="w-8 h-8"
        fill="none"
...
export default function SelectRole() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const { user: syncedUser } = await apiPost("/users/sync", {
      ...
      if (syncedUser?.role === "tenant") {
        navigate("/tenant/dashboard");
      } else if (syncedUser?.role === "manager") {
      ...
      <div className="max-w-3xl w-full">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-dark-500 text-lg max-w-md mx-auto">
            Choose how you'd like to use UrbanRent. You can always change this
            later.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-5 mb-8">
          {roles.map((role) => (
            <button
              ...
              className={`relative text-left p-6 rounded-2xl border-2 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl ${
            ...
            >
              ...
              {/* Icon */}
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-4 ${role.iconBg}`}
              >
                {role.icon}
              </div>
