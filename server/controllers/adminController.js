...
export const adminLogin = async (req, res) => {
  try {
    ...
    if (!email || !accessCode) {
      return res
        .status(400)
        .json({ error: "Email and access code are required." });
    }
    ...
    res.json({
      ...
      sessionToken: createAdminSessionToken(user.clerkId),
      admin: {
        id: user._id,
  ...
  try {
    ...
    const usersWithCounts = await Promise.all(
      users.map(async (u) => {
        ...
        if (u.role === "manager") {
          userData.propertyCount = await Property.countDocuments({
            owner: u._id,
          });
        }
        return userData;
...
export const getPropertyDetail = async (req, res) => {
  try {
    const property = await Property.findById(req.params.id)
      .populate(
        "owner",
        "firstName lastName email avatar phone role createdAt clerkId",
      )
      .lean();
    ...
    const applications = await Application.find({
      ...
      .populate("tenant", "firstName lastName email phone avatar createdAt")
      .lean();
...
export const deleteUser = async (req, res) => {
  try {
    ...
    res.json({
      message: `User ${user.firstName} ${user.lastName} deleted successfully. All login access has been revoked. 7-day property notices have been issued.`,
    });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
export const impersonateUser = async (req, res) => {
  try {
    ...
    res.json({
      ...
      impersonationToken: createImpersonationToken(
        adminUser.clerkId,
        user.clerkId,
      ),
      user: {
    ...
    if (wasSuspending) {
      ...
      if (user.role === "manager") {
        const properties = await Property.find({
          owner: user._id,
          status: { $in: ["active", "rented", "awaiting_payment"] },
        });
      ...
      await Notification.create({
        ...
        message:
          "Your account has been reactivated by the administrator. You now have full access to the platform.",
        type: "success",
      });
...
export const getReactivationRequests = async (req, res) => {
  try {
    ...
    const users = await User.find({
      ...
      .select(
        "firstName lastName email avatar role suspendedAt suspendedReason reactivationRequests isActive",
      )
      .lean();
...
export const getAllApplications = async (req, res) => {
  try {
    ...
    const applications = await Application.find(query)
      ...
      .populate(
        "property",
        "title location.city pricing.monthlyRent images manager",
      )
    ...
    const aggregation = await AdminIncome.aggregate([
      { $match: matchStage },
      {
        $group: {
          ...
          credits: {
            $sum: {
              $cond: [{ $eq: ["$source", "credit_purchase"] }, "$amount", 0],
            },
          },
          commissions: {
            $sum: {
              $cond: [{ $eq: ["$source", "lease_commission"] }, "$amount", 0],
            },
          },
          total: { $sum: "$amount" },
        },
