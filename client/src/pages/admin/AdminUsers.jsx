...
export default function AdminUsers() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      if (type === "delete") {
        ...
        try {
          const impersonationResponse = await apiPost(
            `/admin/users/${user._id}/impersonate`,
          );
          impersonationToken = impersonationResponse.impersonationToken;
          if (!impersonationToken)
            throw new Error("Impersonation token was not issued.");
        } catch (impErr) {
          throw new Error(`Impersonation failed: ${impErr.message}`);
        }

        sessionStorage.setItem("urbanrent_impersonate", user.clerkId);
        sessionStorage.setItem(
          "urbanrent_impersonate_data",
          JSON.stringify({
            name:
        ...
        let target = "";
        const tokenParam = encodeURIComponent(impersonationToken);
        if (user.role === "tenant") {
          target = isDemo
            ? `/demo/tenant/dashboard?impersonate=1&impersonationToken=${tokenParam}`
            : `/tenant/dashboard?impersonate=1&impersonationToken=${tokenParam}`;
        } else {
          target = isDemo
            ? `/demo/manager/dashboard?impersonate=1&impersonationToken=${tokenParam}`
            : `/manager/dashboard?impersonate=1&impersonationToken=${tokenParam}`;
        }
  ...
  const fetchReactivationRequests = useCallback(async () => {
    try {
      ...
      const data = await apiGet(
        `/admin/reactivation-requests?status=${requestFilter}`,
      );
  ...
  const handleReactivationAction = async () => {
    ...
    try {
      await apiPatch(
        `/admin/reactivation-requests/${request.userId}/${request._id}`,
        {
          action,
  ...
  return (
    <AdminLayout
      breadcrumbs={[
        {
          label: "Dashboard",
          href: isDemo ? "/demo/admin" : "/admin/dashboard",
        },
        { label: "Users" },
      ]}
    >
                  ...
                  <>
                    ...
                    <div className="divide-y divide-dark-50">
                      {users.map((user) => (
                        <div
                          key={user._id}
                          className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-4 p-4 items-center hover:bg-dark-50/50 transition-colors"
                        >
                          {/* User Info */}
                          ...
                          <div className="col-span-2">
                            <p
                              className="text-dark-600 text-[11px] truncate"
                              title={user.email}
                            >
                              {user.email}
                            </p>
                          ...
                          <div className="col-span-1">
                            <span
                              className={`inline-block px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase border ${ROLE_BADGE[user.role] || ROLE_BADGE.tenant}`}
                            >
                              {user.role}
                            </span>
                            {user.role === "manager" &&
                              user.propertyCount !== undefined && (
                                <p className="text-dark-400 text-[10px] mt-0.5">
                                  {user.propertyCount} listings
                                </p>
                              )}
                          </div>
                          ...
                          <div className="col-span-1">
                            <span
                              className={`inline-block w-2 h-2 rounded-full ${user.isActive ? "bg-emerald-500" : "bg-red-500"}`}
                            />
                ...
                {requestsLoading ? (
                  ...
                  <div className="py-16 text-center">
                    ...
                    <p className="text-dark-400 text-xs mt-1">
                      Requests from suspended users will appear here
                    </p>
                  </div>
                ) : (
                  <div className="divide-y divide-dark-50">
                    {reactivationRequests.map((request) => (
                      <div
                        key={request._id}
                        className="p-4 hover:bg-dark-50/50 transition-colors"
                      >
                        <div className="flex items-start gap-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              <span
                                className={`inline-block px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase border ${ROLE_BADGE[request.userRole] || ROLE_BADGE.tenant}`}
                              >
                                {request.userRole}
                              </span>
                              <span
                                className={`inline-block px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase border ${REQUEST_STATUS_BADGE[request.status]}`}
                              >
                                {request.status}
                              </span>
                            </div>
                            ...
                            <div className="bg-dark-50 border border-dark-100 rounded-lg p-2.5 mb-2">
                              <p className="text-[10px] font-bold text-dark-500 uppercase tracking-wider mb-0.5">
                                User Message
                              </p>
                              <p className="text-xs text-dark-700 font-medium leading-relaxed">
                                {request.message}
                              </p>
                            </div>
              ...
              <div>
                <h3 className="text-lg font-black text-dark-900 leading-tight">
                  {modal.type === "delete"
                    ...
                    : modal.type === "login"
                      ? "Impersonate User"
                      : `${modal.user.isActive ? "Suspend" : "Activate"} User`}
                </h3>
                <p className="text-sm font-bold text-dark-600">
            ...
            <div className="text-sm text-dark-500 mb-4 bg-dark-50 p-4 rounded-xl border border-dark-100 leading-relaxed font-medium">
              {modal.type === "delete" ? (
                <>
                  ...
                  and immediately revoke all login access. The user will be
                  force-logged out of any active session.
                  {modal.user.role === "manager" &&
                    " All their listed properties will receive a 7-day vacate notice. Affected tenants will be notified."}{" "}
                  If they register again with the same email, a completely new
                  account will be created. This cannot be undone.
                </>
              ) : modal.type === "login" ? (
                <>
                  A new window will open securely logging you into the{" "}
                  {modal.user.role} dashboard as{" "}
                  <span className="text-dark-900 font-bold">
                    {modal.user.firstName}
                  </span>
                  . The user's current active session will be ended
                  automatically and they will be notified. All actions taken
                  will be recorded under your admin session.
                </>
              ) : modal.user.isActive ? (
                <>
                  Are you sure you want to suspend this user? They will not be
                  able to log in or access features. Their properties will
                  receive a 7-day notice.
                </>
              ) : (
                <>
                  Are you sure you want to reactivate this user? They will
                  regain full access to the platform immediately. Suspension
                  notices on their properties will be cleared.
                </>
              )}
            </div>
