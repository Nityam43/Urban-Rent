...
import { Show, UserButton, useUser, SignInButton } from "@clerk/react";
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <a
              href={userRole === "tenant" ? "#top" : "/"}
              className="flex items-center gap-2 group flex-shrink-0"
            >
              <svg
                className={`w-6 h-6 transition-colors duration-300 ${isScrolled ? "text-primary-600" : "text-primary-400"}`}
                fill="currentColor"
                viewBox="0 0 24 24"
              >
              ...
              <span
                className={`text-xl font-bold transition-colors duration-300 ${isScrolled ? "text-dark-900" : "text-white"}`}
              >
            ...
            <div className="hidden md:flex items-center flex-shrink-0 z-10 mx-auto">
              <Show when="signed-out">
                <div className="flex items-center gap-1 bg-slate-700/80 backdrop-blur-sm rounded-full px-1.5 py-1.5">
                  {publicNavLinks.map((link) => {
                    const classes = `px-5 py-2 rounded-full text-sm font-semibold transition-all duration-300 whitespace-nowrap ${
                      link.active
              ...
              <Show when="signed-in">
                {userRole === "tenant" ? (
                  <div
                    className={`flex items-center gap-1 rounded-full px-1.5 py-1.5 transition-all duration-300 ${isScrolled ? "bg-dark-50 shadow-inner border border-dark-100" : "bg-slate-700/80 backdrop-blur-sm"}`}
                  >
                    ...
                    <div className="relative" ref={moreRef}>
                      <button
                        ...
                        className={`flex items-center gap-1 px-5 py-2 rounded-full text-sm font-semibold transition-all duration-300 whitespace-nowrap ${
                          isScrolled
                            ? "text-dark-600 hover:text-dark-900 hover:bg-dark-100"
                            : "text-white/90 hover:text-white hover:bg-white/10"
                        }`}
                  ...
                  <div className="flex items-center gap-1 bg-slate-700/80 backdrop-blur-sm rounded-full px-1 py-1">
                    <span className="px-5 py-2 text-white/90 text-sm font-medium">
                      Urban Rent Platform
                    </span>
                  </div>
                )}
            ...
            <div className="hidden md:flex items-center justify-end gap-3 flex-shrink-0 min-w-[150px]">
              ...
              <Show when="signed-out">
                <SignInButton mode="modal">
                  <button
                    className={`px-5 py-2.5 rounded-full text-sm font-medium transition-all duration-300 cursor-pointer ${isScrolled ? "text-dark-700 hover:text-dark-900 hover:bg-dark-100" : "text-white/90 hover:text-white hover:bg-white/10"}`}
                  >
                    Log in
                  </button>
                </SignInButton>
                <button
                  ...
                  className="bg-primary-600 text-white px-5 py-2.5 rounded-full text-sm font-semibold hover:bg-primary-700 transition-all duration-300 shadow-lg active:scale-95 transform cursor-pointer"
                >
                  Get Started
                </button>
              </Show>
              ...
              <Show when="signed-in">
                {userRole !== "tenant" && (
                  <Link
                    to={dashboardUrl}
                    className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${isScrolled ? "text-primary-600 hover:bg-primary-50" : "text-white hover:bg-white/10"}`}
                  >
                    Dashboard
                  </Link>
                )}
                <div
                  className={`rounded-full p-0.5 h-10 ${isScrolled ? "border shadow-sm border-dark-100" : "bg-white/10"}`}
                >
                  <UserButton
                    afterSignOutUrl="/"
          ...
          {/* Mobile Menu */}
          <div
            className={`md:hidden transition-all duration-300 overflow-hidden ${isMobileMenuOpen ? "max-h-[500px] mt-4" : "max-h-0"}`}
          >
              ...
              <Show when="signed-out">
                {publicNavLinks.map((link) => (
                  <a
                    ...
                    className="block px-4 py-3 rounded-xl text-dark-700 hover:bg-primary-50 hover:text-primary-700 font-medium transition-colors"
                  >
                    {link.label}
                  </a>
                ))}
                  ...
                  <>
                    {TENANT_ALL.map((link) => (
                      <Link
                        ...
                        to={link.path}
                        onClick={() => setIsMobileMenuOpen(false)}
                        className={`block px-4 py-3 rounded-xl text-sm font-medium transition-colors ${link.path === "/" ? "bg-primary-50 text-primary-700" : "text-dark-600 hover:bg-dark-50"}`}
                      >
                        {link.label}
                      </Link>
                    ))}
              ...
              <Show when="signed-out">
                <div className="flex gap-2 pt-2">
                  <SignInButton mode="modal">
                    <button
                      onClick={() => setIsMobileMenuOpen(false)}
                      className="flex-1 px-4 py-2.5 rounded-xl text-dark-700 hover:bg-dark-100 font-medium transition-colors text-center"
                    >
                      Log in
                    </button>
                  </SignInButton>
                  <button
                    ...
                    className="flex-1 px-4 py-2.5 rounded-xl bg-primary-600 text-white font-semibold hover:bg-primary-700 transition-colors text-center"
                  >
                    Get Started
                  </button>
                </div>
