...
const ROLES = [
  {
    ...
    description:
      "Browse verified listings, save favourites, schedule visits, and apply online.",
  ...
  },
  {
    ...
    description:
      "List your properties, find quality tenants, manage bookings, and collect rent.",
...
export default function RoleSelectModal({ isOpen, onClose }) {
  ...
  const redirectUrl =
    selectedRole === "tenant" ? "/tenant/dashboard" : "/manager/dashboard";
      ...
      >
            ...
            <button
              ...
              className={`relative text-left p-4 rounded-xl border-2 transition-all duration-200 ${
                selectedRole === role.id
                  ...
                  : role.inactiveBorder +
                    " border-dark-200 bg-white hover:shadow-sm"
              }`}
            >
              {/* Checkmark */}
              {selectedRole === role.id && (
              ...
              <div
                className={`w-11 h-11 rounded-xl flex items-center justify-center mb-3 ${role.iconBg}`}
              >
                {role.icon}
              </div>
              <h3 className="font-bold text-dark-900 text-sm mb-0.5">
                {role.title}
              </h3>
              ...
              <ul className="mt-2 space-y-1">
                {role.features.map((f, i) => (
                  <li
                    key={i}
                    className="flex items-center gap-1.5 text-[11px] text-dark-500"
                  >
                    <svg
        ...
        {selectedRole ? (
          <SignUpButton mode="modal" forceRedirectUrl={redirectUrl}>
            ...
            >
              Get Started as{" "}
              {selectedRole === "tenant" ? "Tenant" : "Property Manager"}
            </button>
          </SignUpButton>
        ) : (
          <button
