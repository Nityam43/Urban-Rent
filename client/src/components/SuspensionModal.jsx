...
export default function SuspensionModal({
  ...
  const hasPendingRequest = (reactivationRequests || []).some(
    (r) => r.status === "pending",
  );
  const lastRequest = (reactivationRequests || []).sort(
  ...
  const handleSubmitRequest = async () => {
    ...
    if (!clerkId) {
      setSubmitError(
        "Unable to identify your account. Please sign out and try again.",
      );
      return;
    }
        ...
        <div className="px-6 py-5 max-h-[70vh] overflow-y-auto">
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-5">
            ...
            <ul className="text-sm text-amber-700 space-y-1.5 font-medium">
              <li className="flex items-start gap-2">
                ...
                Your properties will be made available to other users after 7
                days
              </li>
              <li className="flex items-start gap-2">
                <span className="w-1 h-1 rounded-full bg-amber-500 mt-2 flex-shrink-0"></span>
          ...
          {hasPendingRequest || submitSuccess ? (
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-4">
              ...
              <p className="text-xs text-blue-700 font-medium">
                ...
                administrator review. You will be notified once a decision has
                been made.
              </p>
              {lastRequest && lastRequest.status === "pending" && (
                <p className="text-[10px] text-blue-500 mt-2 font-medium">
                  Submitted on{" "}
                  {new Date(lastRequest.requestedAt).toLocaleDateString(
                    "en-IN",
            ...
            <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4">
              ...
              <p className="text-xs text-red-700 font-medium">
                {lastRequest.adminResponse ||
                  "Your previous reactivation request was denied by the administrator."}
              </p>
              <button
          ...
          {showRequestForm && !hasPendingRequest && !submitSuccess && (
            <div className="border border-dark-200 rounded-xl p-4 mb-4 bg-dark-50">
              <h4 className="text-sm font-bold text-dark-900 mb-2">
                Help & Support - Reactivation Request
              </h4>
              <p className="text-xs text-dark-500 mb-3 font-medium">
                Explain why your account should be reactivated. Include any
                relevant details that may help the administrator make a
                decision.
              </p>
              <textarea
              ...
              <p className="text-[10px] text-dark-400 mt-1 font-medium">
                {requestMessage.length}/500 characters (minimum 10)
              </p>
